...
function consultarStockActual() {
  for (let i = 0; i < silosDeMaiz.length; i++) {
    ...
    mostrarMensaje(`El ${nombreSilo} tiene ${silo.stockActual} toneladas.`);
  }
}
...
function verificarCalidadMaiz() {
  let destinoMaiz;
  let ingresoMaiz = parseFloat(
    prompt(
      '¿Cuál es el estado del maiz?. Elija una opción (1, 2, 3).\n 1. Maiz en Excelente Estado \n 2. Maiz con Objetos Extraños \n 3. Maiz Húmedo \n '
    )
  );
  ...
  switch (ingresoMaiz) {
    case 1:
      mostrarMensaje(
        'El Maíz está en excelente estado y será derivado al silo de Maiz Bueno'
      );
      destinoMaiz = siloMaizBueno;
      break;
    case 2:
      mostrarMensaje(
        'El Maíz tiene objetos extraños y será derivado al silo de Maiz con CuerposExtraños'
      );
      destinoMaiz = siloMaizCuerposExtraños;
      break;
    case 3:
      mostrarMensaje(
        'El Maíz está Húmedo y será derivado al silo de Maiz Humedo'
      );
      destinoMaiz = siloMaizHumedo;
      break;

    default:
      mostrarMensaje('Opción Invalida. Ingrese una opción válida (1, 2 o 3)');
      destinoMaiz = null;
      break;
  }
...
function agregarMaiz() {
  ...
  if (destinoMaiz !== null) {
    let capacidadDisponible =
      destinoMaiz.capacidadMaxima - destinoMaiz.stockActual;

    let limpieza = prompt('¿Está limpio el silo? (Si/No)').toUpperCase();
    let ventilacion = prompt(
      '¿Tiene buena ventilación el silo? (Si/No)'
    ).toUpperCase();
    ...
    if (maizDescargado <= capacidadDisponible) {
      if (limpieza === 'SI' && ventilacion === 'SI') {
        ...
        mostrarMensaje(
          `Se han recibido ${maizDescargado} toneladas de maiz, que serán almacenadas en el ${destinoMaiz.nombre}.\n El stock actual de maiz en ${destinoMaiz.nombre} es de ${destinoMaiz.stockActual} toneladas.\n El ${destinoMaiz.nombre} se encuentra en óptimas condiciones de almacenamiento.`
        );
      } else {
        mostrarMensaje(
          'No se cumplen las condiciones óptimas de almacenamiento.'
        );
      }
    } else {
      mostrarMensaje(
        `No hay suficiente espacio en el ${destinoMaiz.nombre} para agregar ${maizDescargado}toneladas de maíz. Ingrese una cantidad válida.`
      );
    }
  } else {
    mostrarMensaje(
      'Ingresó una opción inválida. Se ha comunicado el problema al personal superior.'
    );
  }
}
...
function menuMaiz() {
  ...
  do {
    ...
    switch (opcion) {
      ...
      case 3:
        mostrarMensaje(
          `Se comunica al personal superior que los silos se encuentran en las siguientes condiciones: \n El silo de Maíz Bueno tiene ${silosDeMaiz[0].stockActual} toneladas de maíz.\nEl silo de Maíz Cuerpos Extraños tiene ${silosDeMaiz[1].stockActual} toneladas de maíz.\nEl silo de Maíz Húmedo tiene ${silosDeMaiz[2].stockActual} toneladas de maíz. `
        );
        break;
      case 4:
        mostrarMensaje(
          'Gracias por utilizar el sistema de Recepción y Actualización de Stock!!!'
        );
        break;

      default:
        mostrarMensaje(
          'Ha ingresado una opción incorrecta. Por favor ingrese un número del 1 al 4.'
        );
        break;
    }
  } while (opcion != 4);
